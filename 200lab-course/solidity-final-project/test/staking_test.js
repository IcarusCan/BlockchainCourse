...
describe("Staking", function () {
    ...
    let defaultStakeAmount = ethers.utils.parseEther('1000')
    ...
    describe("setReserve", function () {
        ...
        it("should revert if sender isn't contract owner", async function () {
          await expect(staking.connect(staker2).setReserve(address0)).to.be.revertedWith("Ownable: caller is not the owner")
        })
    ...
    describe("addStakePackage", function () {
        it("should revert if rate_ = 0", async function () {
          await expect(staking.addStakePackage(0, defaultDecimal, defaultMinStaking, oneYear)).to.be.revertedWith("Staking: Invalid rate")
        })
        it("should revert if minStaking_ = 0", async function () {
          await expect(staking.addStakePackage(defaultRate, defaultDecimal, 0, oneYear)).to.be.revertedWith("Staking: Invalid minStaking_ amount")
        })
        it("should revert if lockTime_ = 0", async function () {
          await expect(staking.addStakePackage(defaultRate, defaultDecimal, defaultMinStaking, 0)).to.be.revertedWith("Staking: Invalid lockTime_")
        })
    ...
    describe("removeStakePackage", function () {
        ...
        it("should revert if stake package was offline", async function () {
          ...
          await expect(staking.removeStakePackage(1)).to.be.revertedWith("Staking: Package is offline")
    ...
    describe("stake", function () {
        ...
        it("should revert if amount < min staking", async function () {
          await expect(staking.stake(defaultMinStaking.sub(1), 1)).to.be.revertedWith("Staking: stake amount must be greater than min stake")
        })
        ...
        it("should add stake correctly to a new stake info ", async function () {
          ...
          await expect(stakeTx).to.emit(staking, 'StakeUpdate').withArgs(staker.address, 1, defaultStakeAmount, 0);
        })
    ...
    describe("unStake", function () {
        ...
        it("should revert if not reach lock time", async function () {
          ...
          await expect(staking.connect(staker).unStake(1)).to.be.revertedWith("Staking: lockTime is not enough")
        })
